...
### 🐛 Bug Fixes  问题修复
- 【AI 抠图·发丝细节】`vision::matting` 距离场骨架阈值由软阈值主阈值（128）改为**存在阈值**（`SKELETON_EXIST_ALPHA = 16`）：此前软阈值过渡带产出的淡发丝 alpha（如 prob≈0.47 → alpha≈66）低于 128 被判为背景，经距离场「外部压灭」系数后归零，细发丝被整根削掉；改判存在阈值后淡发丝进入前景骨架，距离场仅负责主体恒 255 / 背景恒 0 / 过渡带保留软 alpha
- 【AI 抠图·半透明边缘】`workflow::MASK_SOFT_RANGE`（soft-level 过渡带带宽）由 48 调至 64：过渡带下界由 104 下探到 96，0.38~0.5 概率的淡发丝/薄纱不再被 soft-level 直接清零
- 【AI 抠图·mask 旋转插值】`vision::affine::rotate_translate_image_same` 对 mask 的 warp 插值由**最近邻**改为**双线性**：最近邻会把旋转后的半透明发丝断成硬边锯齿，双线性保持 alpha 连续性

### 📈 Improvements 性能/体验优化
- 【换底质量】e2e 量化对比（以 task_88/16.jpg 为基线）：过渡带内发丝像素占比 1.93% → 3.35%（+74%），漏底色/halo 占比 7.05% → 2.11%（约 70% 改善）；多图抽样 halo 降至 0%~3.6%

### 📚 Docs 文档更新
- 补充抠图/换底质量专项说明（存在阈值骨架、soft-level 带宽、mask 双线性插值），见 `docs/07-能力增强.md`
