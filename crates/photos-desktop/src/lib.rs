...
fn spawn_server(cfg: Config) -> Result<String, Box<dyn std::error::Error>> {
    ...
    let (engine_factory, engine_pool) = photos_api::engine_factory_from_env(&cfg)?;
    let app = photos_api::router_with_frontend(cfg, engine_factory, true, engine_pool, frontend);
